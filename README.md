...
- `export(drive_id)` — export Drive markdown to `./output/{drive_id}-vertex-rag.md`.
...
- Optional: set `VERTEX_RAG_MCP_SKIP_OWNERS` to a comma-separated list of owner identifiers to skip during indexing/export (matches owner email or display name).
